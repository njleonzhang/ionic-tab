...
gulp.task('make-bundle', ['del', 'html2js', 'css2js'], function () {
  return gulp.src(['dist/*', './src/*.js'])
    .pipe(concat('ionSimpleTab.bundle.min.js'))
    .pipe(uglify())
    .pipe(gulp.dest('dist/'));
});

gulp.task('make-bundle-dev', ['del', 'html2js', 'css2js'], function () {
  return gulp.src(['dist/*', './src/*.js'])
    .pipe(concat('ionSimpleTab.bundle.js'))
    .pipe(gulp.dest('dist/'));
});

gulp.task('del-temp-files', ['make-bundle', 'make-bundle-dev'], function () {
  del(['dist/templates.js', 'dist/tabSet.js']);
});
